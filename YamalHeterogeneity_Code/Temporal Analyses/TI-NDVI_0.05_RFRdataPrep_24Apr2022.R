...
TINDVI_SS <- readRDS(file = "TI-NDVI_0.05_Updated/TINDVI0.05_Slope_27Feb2022.RDS")
TINDVI_PixelIDs <- readRDS(file = "TI-NDVI_0.05_Updated/TINDVI_Clean_Coords_27Feb2022.RDS")
TINDVI_SS_PIs <- cbind(TINDVI_PixelIDs, TINDVI_SS)
TINDVI_SS_PIs <- subset(TINDVI_SS_PIs, select = -c(x,y) )
...
TINDVI_v1 <- TINDVI_SS_PIs[(TINDVI_SS_PIs$PixelID %in% SoilText_PIs$PixelID),]
VegType_v1 <- VegType_PIs[(VegType_PIs$PixelID %in% SoilText_PIs$PixelID),]

#TI-NDVI now has the fewest pixels (300,062)
CoastDist_v2 <- CoastDist_v1[(CoastDist_v1$PixelID %in% TINDVI_v1$PixelID),]
Elevation_v2 <- Elevation_v1[(Elevation_v1$PixelID %in% TINDVI_v1$PixelID),]
HumanMod_v2 <- HumanMod_v1[(HumanMod_v1$PixelID %in% TINDVI_v1$PixelID),]
LandAge_v2 <- LandAge_v1[(LandAge_v1$PixelID %in% TINDVI_v1$PixelID),]
MGSP_v2 <- MGSP_v1[(MGSP_v1$PixelID %in% TINDVI_v1$PixelID),]
SFPO_v2 <- SFPO_v1[(SFPO_v1$PixelID %in% TINDVI_v1$PixelID),]
SM_v2 <- SM_v1[(SM_v1$PixelID %in% TINDVI_v1$PixelID),]
ST_v2 <- SoilText_PIs[(SoilText_PIs$PixelID %in% TINDVI_v1$PixelID),]
SubChem_v2 <- SubChem_PIs[(SubChem_PIs$PixelID %in% TINDVI_v1$PixelID),]
SWI_v2 <- SWI_v1[(SWI_v1$PixelID %in% TINDVI_v1$PixelID),]
VegType_v2 <- VegType_v1[(VegType_v1$PixelID %in% TINDVI_v1$PixelID),]

#Elevation now has the fewest pixels (299,399)
...
TINDVI_v3 <- TINDVI_v1[(TINDVI_v1$PixelID %in% Elevation_v2$PixelID),]
VegType_v3 <- VegType_v2[(VegType_v2$PixelID %in% Elevation_v2$PixelID),]

#Veg type now has the fewest pixels (299,131)
...
#MGSP now has the fewest pixels (298,950)
...
TINDVI_v6 <- TINDVI_v5[(TINDVI_v5$PixelID %in% MGSP_v5$PixelID),]
VegType_v6 <- VegType_v5[(VegType_v5$PixelID %in% MGSP_v5$PixelID),]

#Land age now has the fewest pixels (298,945)
...
TINDVI_v7 <- TINDVI_v6[(TINDVI_v6$PixelID %in% LandAge_v6$PixelID),]
VegType_v7 <- VegType_v6[(VegType_v6$PixelID %in% LandAge_v6$PixelID),]

#Remove PixelID columns from the final datasets
CoastDist_v7 <- subset(CoastDist_v7, select = -c(PixelID) )
Elevation_v7 <- subset(Elevation_v7, select = -c(PixelID) )
HumanMod_v7 <- subset(HumanMod_v7, select = -c(PixelID) )
LandAge_v6 <- subset(LandAge_v6, select = -c(PixelID) )
MGSP_v7 <- subset(MGSP_v7, select = -c(PixelID) )
SFPO_v7 <- subset(SFPO_v7, select = -c(PixelID) )
SM_v7 <- subset(SM_v7, select = -c(PixelID) )
ST_v7 <- subset(ST_v7, select = -c(PixelID) )
SubChem_v7 <- subset(SubChem_v7, select = -c(PixelID) )
SWI_v7 <- subset(SWI_v7, select = -c(PixelID) )
TINDVI_v7 <- subset(TINDVI_v7, select = -c(PixelID) )
VegType_v7 <- subset(VegType_v7, select = -c(PixelID) )

#Datasets now have same number of matching pixels. Combine into 1 dataframe and rename columns
RFRinput <-cbind(TINDVI_v7, CoastDist_v7, Elevation_v7, HumanMod_v7, LandAge_v6, MGSP_v7, SFPO_v7, 
                 SM_v7, ST_v7, SubChem_v7, SWI_v7, VegType_v7)
...
MGSP_IQR <- 0.55030 - -0.38594
MGSP_LB <- -0.38594 - (1.5*MGSP_IQR)
MGSP_UB <- 0.55030 + (1.5 *MGSP_IQR)
...
SWI_IQR <- 0.28164 - 0.06083
SWI_LB <- 0.06083 - (1.5*SWI_IQR)
SWI_UB <- 0.28164 + (1.5 *SWI_IQR)
...
saveRDS(RFRinput, file="RFRinput_TINDVI0.05_24Apr2022.RDS")
write.table(RFRinput, file="RFRinput_TINDVI0.05_24Apr2022.csv", sep=",")
...
SlopesForCor <- read_excel("RFR_TINDVI0.5_Update/RFRinput_TINDVI0.05_24Apr2022.xlsx", 
                           na = "NA")
