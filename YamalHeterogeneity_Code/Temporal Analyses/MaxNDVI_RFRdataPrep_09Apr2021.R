...
MaxNDVI_SS_PIs <- subset(MaxNDVI_SS_PIs, select = -c(x,y) )
...
#Start with soil texture, which has the fewest pixels (305,136)
#Remove pixel IDs from dataframes NOT present in the soil texture dataframe, and vice versa.
CoastDist_v1 <- CoastDist_PIs[(CoastDist_PIs$PixelID %in% SoilText_PIs$PixelID),]
Elevation_v1 <- Elevation_PIs[(Elevation_PIs$PixelID %in% SoilText_PIs$PixelID),]
HumanMod_v1 <- HumanMod_PIs[(HumanMod_PIs$PixelID %in% SoilText_PIs$PixelID),]
LandAge_v1 <- LandAge_PIs[(LandAge_PIs$PixelID %in% SoilText_PIs$PixelID),]
MaxNDVI_v1 <- MaxNDVI_SS_PIs[(MaxNDVI_SS_PIs$PixelID %in% SoilText_PIs$PixelID),]
MGSP_v1 <- MGSP_SS_PIs[(MGSP_SS_PIs$PixelID %in% SoilText_PIs$PixelID),]
SFPO_v1 <- SFPO_SS_PIs[(SFPO_SS_PIs$PixelID %in% SoilText_PIs$PixelID),]
SM_v1 <- SM_SS_PIs[(SM_SS_PIs$PixelID %in% SoilText_PIs$PixelID),]
SubChem_v1 <- SubChem_PIs[(SubChem_PIs$PixelID %in% SoilText_PIs$PixelID),]
SWI_v1 <- SWI_SS_PIs[(SWI_SS_PIs$PixelID %in% SoilText_PIs$PixelID),]
VegType_v1 <- VegType_PIs[(VegType_PIs$PixelID %in% SoilText_PIs$PixelID),]

#Max NDVI now has the fewest pixels (301,787)
#Remove pixel IDs from dataframes NOT present in the Max NDVI dataframe, and vice versa.
CoastDist_v2 <- CoastDist_v1[(CoastDist_v1$PixelID %in% MaxNDVI_v1$PixelID),]
Elevation_v2 <- Elevation_v1[(Elevation_v1$PixelID %in% MaxNDVI_v1$PixelID),]
HumanMod_v2 <- HumanMod_v1[(HumanMod_v1$PixelID %in% MaxNDVI_v1$PixelID),]
LandAge_v2 <- LandAge_v1[(LandAge_v1$PixelID %in% MaxNDVI_v1$PixelID),]
MGSP_v2 <- MGSP_v1[(MGSP_v1$PixelID %in% MaxNDVI_v1$PixelID),]
SFPO_v2 <- SFPO_v1[(SFPO_v1$PixelID %in% MaxNDVI_v1$PixelID),]
SM_v2 <- SM_v1[(SM_v1$PixelID %in% MaxNDVI_v1$PixelID),]
ST_v2 <- SoilText_PIs[(SoilText_PIs$PixelID %in% MaxNDVI_v1$PixelID),]
SubChem_v2 <- SubChem_v1[(SubChem_v1$PixelID %in% MaxNDVI_v1$PixelID),]
SWI_v2 <- SWI_v1[(SWI_v1$PixelID %in% MaxNDVI_v1$PixelID),]
VegType_v2 <- VegType_v1[(VegType_v1$PixelID %in% MaxNDVI_v1$PixelID),]

#Elevation now has the fewest pixels (301,046)
...
MaxNDVI_v3 <- MaxNDVI_v1[(MaxNDVI_v1$PixelID %in% Elevation_v2$PixelID),]
...
ST_v3 <- ST_v2[(ST_v2$PixelID %in% Elevation_v2$PixelID),]
...
#VegType now has the fewest pixels (300,747)
...
MaxNDVI_v4 <- MaxNDVI_v3[(MaxNDVI_v3$PixelID %in% VegType_v3$PixelID),]
...
#Human mod now has the fewest pixels (300,572)
...
#MGSP now has the fewest pixels (300,706)
...
#Land age now has the fewest pixels (300,699)
#Remove pixel IDs from dataframes NOT present in the land age dataframe, and vice versa.
CoastDist_v7 <- CoastDist_v6[(CoastDist_v6$PixelID %in% LandAge_v6$PixelID),]
Elevation_v7 <- Elevation_v6[(Elevation_v6$PixelID %in% LandAge_v6$PixelID),]
HumanMod_v7 <- HumanMod_v6[(HumanMod_v6$PixelID %in% LandAge_v6$PixelID),]
MGSP_v7 <- MGSP_v5[( MGSP_v5$PixelID %in% LandAge_v6$PixelID),]
MaxNDVI_v7 <- MaxNDVI_v6[(MaxNDVI_v6$PixelID %in% LandAge_v6$PixelID),]
SFPO_v7 <- SFPO_v6[(SFPO_v6$PixelID %in% LandAge_v6$PixelID),]
SM_v7 <- SM_v6[(SM_v6$PixelID %in% LandAge_v6$PixelID),]
ST_v7 <- ST_v6[(ST_v6$PixelID %in% LandAge_v6$PixelID),]
SubChem_v7 <- SubChem_v6[(SubChem_v6$PixelID %in% LandAge_v6$PixelID),]
SWI_v7 <- SWI_v6[(SWI_v6$PixelID %in% LandAge_v6$PixelID),]
VegType_v7 <- VegType_v6[(VegType_v6$PixelID %in% LandAge_v6$PixelID),]

#SWI now has the fewest pixels (300,694)
#Remove pixel IDs from dataframes NOT present in the SWI dataframe, and vice versa.
CoastDist_v8 <- CoastDist_v7[(CoastDist_v7$PixelID %in% SWI_v7$PixelID),]
Elevation_v8 <- Elevation_v7[(Elevation_v7$PixelID %in% SWI_v7$PixelID),]
HumanMod_v8 <- HumanMod_v7[(HumanMod_v7$PixelID %in% SWI_v7$PixelID),]
LandAge_v8 <- LandAge_v6[(LandAge_v6$PixelID %in% SWI_v7$PixelID),]
MGSP_v8 <- MGSP_v7[( MGSP_v7$PixelID %in% SWI_v7$PixelID),]
MaxNDVI_v8 <- MaxNDVI_v7[(MaxNDVI_v7$PixelID %in% SWI_v7$PixelID),]
SFPO_v8 <- SFPO_v7[(SFPO_v7$PixelID %in% SWI_v7$PixelID),]
SM_v8 <- SM_v7[(SM_v7$PixelID %in% SWI_v7$PixelID),]
ST_v8 <- ST_v7[(ST_v7$PixelID %in% SWI_v7$PixelID),]
SubChem_v8 <- SubChem_v7[(SubChem_v7$PixelID %in% SWI_v7$PixelID),]
VegType_v8 <- VegType_v7[(VegType_v7$PixelID %in% SWI_v7$PixelID),]

#Export one dataset for Pixel IDs
#saveRDS(MaxNDVI_v8, file="MaxNDVI_RFR_PixelIDs.RDS")

#Remove PixelID columns from the final datasets
CoastDist_v8 <- subset(CoastDist_v8, select = -c(PixelID) )
Elevation_v8 <- subset(Elevation_v8, select = -c(PixelID) )
HumanMod_v8 <- subset(HumanMod_v8, select = -c(PixelID) )
LandAge_v8 <- subset(LandAge_v8, select = -c(PixelID) )
MGSP_v8 <- subset(MGSP_v8, select = -c(PixelID) )
MaxNDVI_v8 <- subset(MaxNDVI_v8, select = -c(PixelID) )
SFPO_v8 <- subset(SFPO_v8, select = -c(PixelID) )
SM_v8 <- subset(SM_v8, select = -c(PixelID) )
ST_v8 <- subset(ST_v8, select = -c(PixelID) )
SubChem_v8 <- subset(SubChem_v8, select = -c(PixelID) )
SWI_v7 <- subset(SWI_v7, select = -c(PixelID) )
VegType_v8 <- subset(VegType_v8, select = -c(PixelID) )

#Datasets now have same number of matching pixels. Combine into 1 dataframe and rename columns
RFRinput <-cbind(MaxNDVI_v8, CoastDist_v8, Elevation_v8, HumanMod_v8, LandAge_v8, MGSP_v8, SFPO_v8,
                 SM_v8, ST_v8, SubChem_v8, SWI_v7, VegType_v8)
...
SWI_IQR <- 0.28177 - 0.06109
SWI_LB <- 0.06109 - (1.5*SWI_IQR)
SWI_UB <- 0.28177 + (1.5 *SWI_IQR)

#Determine number of pixels with SWI slopes >= 0.5
SWI_GTE05 <- RFRinput[(RFRinput$SummerWarmth >= 0.5),]

summary(RFRinput$SoilMoisture)
SM_IQR <- -0.1507 - -1.3347
SM_LB <- -1.3347 - (1.5*SM_IQR)
SM_UB <- -0.1507 + (1.5 *SM_IQR)
...
MGSP_IQR <- 0.55002 - -0.38583
MGSP_LB <- -0.38583 - (1.5*MGSP_IQR)
MGSP_UB <- 0.55002 + (1.5 *MGSP_IQR)
...
#saveRDS(RFRinput, file="RFRinput_MaxNDVI_09Apr2021.RDS")
#write.table(RFRinput, file="RFRinput_MaxNDVI_09Apr2021.csv", sep=",")
...
SlopesForCor <- read_excel("RFR_Final/MaxNDVI/RFRinput_MaxNDVI_09Apr2021.xlsx", 
                           na = "NA")
