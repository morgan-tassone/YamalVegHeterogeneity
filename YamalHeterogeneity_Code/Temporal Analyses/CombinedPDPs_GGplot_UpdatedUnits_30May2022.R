...
####Max NDVI####
#Load final model
MaxNDVI_RFRmodel <- get(load("RFR_Final/MaxNDVI/25_Bootstrap/MaxNDVI_UpdatedRFRmodel_25boot_09Apr2021.Rdata"))

####Max NDVI - Coast Distance (not outliers included)
Max_CoastDist_PDP <- readRDS(file = "RFR_Final/MaxNDVI/25_Bootstrap/MaxNDVI_CoastDist_PDPdata_09Apr2021.RDS")
...
####Max NDVI - human mod
Max_HumanMod_PDP <- readRDS(file = "RFR_Final/MaxNDVI/25_Bootstrap/MaxNDVI_HumanMod_PDPdata_09Apr2021.RDS")
...
Max_SoilMoisture_PDP <- readRDS(file = "RFR_Final/MaxNDVI/25_Bootstrap/MaxNDVI_SoilMoisture_PDPdata_09Apr2021.RDS")
...
Max_Precip_PDP <- readRDS(file = "RFR_Final/MaxNDVI/25_Bootstrap/MaxNDVI_Precip_PDPdata_09Apr2021.RDS")
Max_Precip_PDP$yhat <- Max_Precip_PDP$yhat * 10000
#add ID points to segment data for graphing
Max_Precip_PDP$PointID <- seq.int(nrow(Max_Precip_PDP))

MaxNDVI_Precip_PosOutliers <- Max_Precip_PDP[(Max_Precip_PDP$Precipitation > 1.953795),]
MaxNDVI_Precip_NegOutliers <- Max_Precip_PDP[(Max_Precip_PDP$Precipitation < -1.789605),]
#Keep only data values not included in the above datasets
...
####Max NDVI - elevation
Max_Elevation_PDP <- readRDS(file = "RFR_Final/MaxNDVI/25_Bootstrap/MaxNDVI_Elevation_PDPdata_09Apr2021.RDS")
Max_Elevation_PDP$yhat <- Max_Elevation_PDP$yhat * 10000
#add ID points to segment data for graphing
Max_Elevation_PDP$PointID <- seq.int(nrow(Max_Elevation_PDP))

MaxNDVI_Elevation_PosOutliers <- Max_Elevation_PDP[(Max_Elevation_PDP$Elevation > 63.5),]
MaxNDVI_Elevation_NegOutliers <- Max_Elevation_PDP[(Max_Elevation_PDP$Elevation < -36.5),]
#Keep only data values not included in the above datasets
...
####Max NDVI - SWI
Max_SWI_PDP <- readRDS(file = "RFR_Final/MaxNDVI/25_Bootstrap/MaxNDVI_SummerWarmth_PDPdata_09Apr2021.RDS")
Max_SWI_PDP$yhat <- Max_SWI_PDP$yhat * 1000
#add ID points to segment data for graphing
Max_SWI_PDP$PointID <- seq.int(nrow(Max_SWI_PDP))

MaxNDVI_SWI_PosOutliers <- Max_SWI_PDP[(Max_SWI_PDP$SummerWarmth > 0.61279),]
MaxNDVI_SWI_NegOutliers <- Max_SWI_PDP[(Max_SWI_PDP$SummerWarmth < -0.26993),]
#Keep only data values not included in the above datasets
...
Max_SWI_plot <- ggplot() +
  ...
  #use stat smooth to get a smooth line between data points
  #stat_smooth(aes(y=yhat, x=CoastDist), method = lm, formula = y ~ poly(x, 10), se = FALSE) +
  labs(x = expression(paste("Summer Warmth Index Slope (°C months ",yr^-1,")")),
       y = expression(paste("Max NDVI  ",yr^-1," (x ",10^-3,")"))) +
  #scale_y_continuous(breaks = seq(6.0,8.5,0.5)) +
  theme_bw() +
  theme(axis.text = element_text(size = 12, color = "black"),
...
####TI-NDVI####
#Load final model
TINDVI_RFRmodel <- get(load("RFR_TINDVI0.5_Update/TI-NDVI_UpdatedRFRmodel_25boot_24Apr2022.Rdata"))

####TI-NDVI - Coast Distance (no outliers included)
TI_CoastDist_PDP <- readRDS(file = "RFR_TINDVI0.5_Update/TI-NDVI_CoastDist_PDPdata_24Apr2022.RDS")
...
####TI-NDVI - Precip
TI_Precip_PDP <- readRDS(file = "RFR_TINDVI0.5_Update/TI-NDVI_Precip_PDPdata_24Apr2022.RDS")
TI_Precip_PDP$yhat <- TI_Precip_PDP$yhat * 100
#add ID points to segment data for graphing
TI_Precip_PDP$PointID <- seq.int(nrow(TI_Precip_PDP))

TINDVI_Precip_PosOutliers <- TI_Precip_PDP[(TI_Precip_PDP$Precipitation > 1.95466),]
TINDVI_Precip_NegOutliers <- TI_Precip_PDP[(TI_Precip_PDP$Precipitation < -1.7903),]
#Keep only data values not included in the above datasets
...
####TI-NDVI - Elevation
TI_Elevation_PDP <- readRDS(file = "RFR_TINDVI0.5_Update/TI-NDVI_Elevation_PDPdata_24Apr2022.RDS")
TI_Elevation_PDP$yhat <- TI_Elevation_PDP$yhat * 100
#add ID points to segment data for graphing
TI_Elevation_PDP$PointID <- seq.int(nrow(TI_Elevation_PDP))

TINDVI_Elevation_PosOutliers <- TI_Elevation_PDP[(TI_Elevation_PDP$Elevation > 63.5),]
TINDVI_Elevation_NegOutliers <- TI_Elevation_PDP[(TI_Elevation_PDP$Elevation < -36.5),]
#Keep only data values not included in the above datasets
...
####TI NDVI - Human mod
TI_HumanMod_PDP <- readRDS(file = "RFR_TINDVI0.5_Update/TI-NDVI_HumanMod_PDPdata_24Apr2022.RDS")
...
####TI-NDVI - SWI
TI_SWI_PDP <- readRDS(file = "RFR_TINDVI0.5_Update/TI-NDVI_SummerWarmth_PDPdata_24Apr2022.RDS")
TI_SWI_PDP$yhat <- TI_SWI_PDP$yhat * 100
#add ID points to segment data for graphing
TI_SWI_PDP$PointID <- seq.int(nrow(TI_SWI_PDP))

TINDVI_SWI_PosOutliers <- TI_SWI_PDP[(TI_SWI_PDP$SummerWarmth > 0.61285),]
TINDVI_SWI_NegOutliers <- TI_SWI_PDP[(TI_SWI_PDP$SummerWarmth < -0.2703),]
#Keep only data values not included in the above datasets
...
TI_SoilMoisture_PDP <- readRDS(file = "RFR_TINDVI0.5_Update/TI-NDVI_SoilMoisture_PDPdata_24Apr2022.RDS")
