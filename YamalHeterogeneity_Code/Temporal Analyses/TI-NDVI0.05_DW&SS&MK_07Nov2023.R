...
#Re-import exported MK data
TINDVI_MKsl <- readRDS(file = "TI-NDVI_0.05_Updated/TINDVI0.05_MKSS_27Feb2022.RDS")
...
####Isolate pixels with significant results
library(tidyverse)

TINDVI_sig_only <- TINDVI_MKresult %>%
  rename(sens = `Sen's slope`,
         pval = V1) %>%
  filter(pval < 0.05) %>%
  data.frame() %>%
  select(-x, -y)
...
saveRDS(TINDVI_sig_only, file="UpdatedTINDVI_SigOnly_07Nov2023.RDS")
